...
def download_via_archive(url, attempt=0):
    ...
    req = requests.post("https://web.archive.org/save", headers=headers, data=data)
    ...
    while done == False:
        ...
        statusReq = requests.get(
            "https://web.archive.org/save/status/%s" % job_id, headers=headers
        )
    ...
    while i < 5:
        i += 1
        time.sleep(2)
        urlCheck = requests.get(
            "https://archive.org/wayback/available?url=%s" % original_url,
            headers=headers,
        )
