...
const header = fs.readFileSync(__dirname + '/html-report-header.html', {
  encoding: 'utf-8',
});
const footer = fs.readFileSync(__dirname + '/html-report-footer.html', {
  encoding: 'utf-8',
});

if (process.argv.length < 3) {
  console.error(
    'Usage: node tools/generate-html-sentences-report.js <days back to go> [branch] > out.html'
  );
  process.exit(1);
}
...
async function getObjects({ bucketName }) {
  ...
  try {
    ...
    process.stdout.write(JSON.stringify(objectPages.flat(), null, 2));
